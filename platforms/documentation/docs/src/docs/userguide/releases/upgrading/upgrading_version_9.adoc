...
Gradle now publishes only the checksums that repositories universally expect: `.sha1` and `.md5` for Maven, and `.sha1` for Ivy.

These extra checksums were rarely consumed and frequently rejected by repositories that do not recognize them, so publishing them added noise without a corresponding benefit.
Consuming builds are unaffected: Gradle has never required SHA-256 or SHA-512 checksums to be present in order to resolve or verify artifacts.

If you need SHA-256 or SHA-512 checksums for published artifacts, generate them as part of your publishing pipeline.
